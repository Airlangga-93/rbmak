...
<head>
    ...
    <style>
        ...
        .loader-logo { width: 80px; margin-bottom: 20px; animation: pulse 1.5s infinite ease-in-out; }
        .loader-bar { width: 150px; height: 4px; background: #f3f3f3; border-radius: 10px; overflow: hidden; position: relative; }
        .loader-bar::after {
            content: ''; width: 40%; height: 100%; background: var(--accent);
            position: absolute; left: -100%; animation: loading 1.5s infinite ease;
        }
    ...
    <div x-data="{ searchModalOpen: false, mobileMenuOpen: false }" @keydown.escape.window="searchModalOpen = false">
        ...
        <div x-show="searchModalOpen" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="relative min-h-screen flex items-start justify-center pt-32 px-4">
                <div @click.away="searchModalOpen = false" class="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden">
                    <form action="{{ route('search') }}" method="GET" class="relative">
                        <input type="text" name="query" class="w-full text-xl py-6 pl-14 pr-6 focus:ring-0 border-none outline-none font-sans"
        ...
        <header class="fixed top-0 z-50 w-full shadow-sm bg-white">
            <div class="max-w-screen-xl mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-8 py-3">
                <a href="/" class="flex-shrink-0 flex items-center space-x-2 md:space-x-3 group">
                    <img src="{{ asset('assets/img/image.png') }}" alt="Logo PT RBM" class="h-10 w-auto md:h-12 transition-transform group-hover:scale-105">
                    <div class="flex flex-col border-l border-gray-200 pl-2 md:pl-3">
                        <span class="text-rbm-dark font-times text-[11px] sm:text-sm md:text-lg font-bold leading-tight uppercase tracking-tighter">
                            PT RIZQALLAH BOER MAKMUR
                        </span>
                        <span class="text-[8px] md:text-xs font-sans text-gray-500 font-medium tracking-widest uppercase opacity-80">
                ...
                <div class="hidden lg:flex items-center space-x-6">
                    <button @click="searchModalOpen = true" class="text-gray-500 hover:text-rbm-accent transition-all p-2.5 bg-gray-100 rounded-full">
                        <i class="fa-solid fa-magnifying-glass text-base"></i>
                    </button>
                    <a href="https://wa.me/{{ $whatsappNumber }}?text={{ urlencode($whatsappMessage) }}" target="_blank" rel="noopener noreferrer"
                ...
                <div class="lg:hidden flex items-center space-x-2">
                    ...
                    <button @click="mobileMenuOpen = !mobileMenuOpen" class="text-2xl text-rbm-dark p-2 focus:outline-none">
                ...
                <div class="max-w-screen-xl mx-auto flex items-center justify-center gap-x-6 xl:gap-x-10 px-4 h-12">
                    ...
                    <div class="relative group h-full flex items-center">
                        ...
                        <div class="absolute top-full dropdown-content bg-white shadow-xl border border-gray-100 rounded-b-xl py-2 w-52 overflow-hidden">
                            ...
                            <a href="{{ route('partners.index') }}" class="block px-5 py-3 text-sm text-gray-600 hover:bg-orange-50 hover:text-rbm-accent transition-colors">Our Customer</a>
            ...
            {{-- MOBILE MENU PANEL --}}
            <div x-show="mobileMenuOpen" x-cloak x-transition class="lg:hidden bg-white w-full border-b border-gray-200 shadow-xl overflow-y-auto max-h-[80vh]">
                <div class="flex flex-col p-5 space-y-1">
                    ...
                    <div x-data="{ open: false }">
                        ...
                        <div x-show="open" class="bg-gray-50 rounded-xl mx-2 my-1 overflow-hidden">
                            <a href="{{ route('about') }}" class="block px-6 py-3 text-sm text-gray-600 font-medium">Tentang Kami</a>
                            <a href="{{ route('gallery.index') }}" class="block px-6 py-3 text-sm text-gray-600 font-medium">Galeri Proyek</a>
                            <a href="{{ route('partners.index') }}" class="block px-6 py-3 text-sm text-gray-600 font-medium">Mitra Industri</a>
                            <a href="{{ route('testimonials.index') }}" class="block px-6 py-3 text-sm text-gray-600 font-medium">Testimonial</a>
                        </div>
                    </div>
        ...
        <main class="flex flex-col min-h-[60vh]">
            ...
            <div class="fixed bottom-6 right-6 z-40 flex flex-col gap-3">
                <button x-data="{ shown: false }" x-init="window.addEventListener('scroll', () => { shown = window.scrollY > 400 })" 
                        x-show="shown" x-transition @click="window.scrollTo({ top: 0, behavior: 'smooth' })"
            ...
            <div class="max-w-screen-xl mx-auto px-6 sm:px-8 lg:px-8 pb-16">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
                    ...
                    <div>
                        ...
                        <ul class="space-y-4 text-sm text-rbm-light-text font-medium">
                            <li><a href="{{ route('faq') }}" class="hover:text-white transition-colors flex items-center gap-2"><i class="fa-solid fa-chevron-right text-[10px] text-rbm-accent"></i> FAQ</a></li>
                            <li>
                                <a href="https://wa.me/{{ $whatsappNumber }}" target="_blank" rel="noopener noreferrer" class="hover:text-white transition-colors flex items-center gap-2 group">
                                    <i class="fa-solid fa-chevron-right text-[10px] text-rbm-accent group-hover:translate-x-1 transition-transform"></i>
                    ...
                    <div>
                        ...
                        <div class="space-y-5 text-sm text-rbm-light-text">
                            <div class="flex gap-4">
                                ...
                                <span class="leading-relaxed text-[13px]">Menara Palma Lantai 12, Jakarta Selatan 12950</span>
                            </div>
                            <div class="flex gap-4">
                                <i class="fas fa-map-marker-alt text-rbm-accent text-lg mt-1"></i>
                                <span class="leading-relaxed text-[13px]">Pamulihan Sumedang 45365</span>
                            </div>
