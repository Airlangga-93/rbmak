                        ...
                        @forelse($products as $product)
                            <tr class="border-b border-gray-200 hover:bg-gray-50 transition-colors duration-200 product-row">
                                ...
                                <td class="py-4 px-4 text-left">
                                    @if($product->image)
                                        @php
                                            // LOGIKA UNTUK MENANGANI BERBAGAI SUMBER GAMBAR
                                            // 1. Cek apakah ini file seeder di folder assets
                                            if (str_contains($product->image, 'assets/')) {
                                                $path = asset($product->image);
                                            }
                                            // 2. Cek apakah ini file upload baru di folder storage
                                            else {
                                                $path = asset('storage/' . $product->image);
                                            }
                                        @endphp
                                        <img src="{{ $path }}" alt="{{ $product->name }}" class="w-12 h-12 object-cover rounded-lg border shadow-sm bg-white"
