...
@php
    // Memproses data gambar di sisi server agar tidak pecah dan siap digunakan Alpine.js
    $galleryData = $galleries->map(function($item) {
        if (str_starts_with($item->image, 'assets/')) {
            $path = asset($item->image);
        } elseif (str_starts_with($item->image, 'gallery/')) {
        ...
        return [
            'id' => $item->id,
            'cat' => strtoupper($item->category ?? 'Proyek'),
            'img' => $path,
            'title' => $item->title ?? 'Dokumentasi PT. RBM'
        ];
    });
@endphp
...
<div class="bg-white min-h-screen font-sans text-[#161f36] selection:bg-[#FF7518]/30"
     x-data="{
        items: {{ $galleryData->toJson() }},
        activeSlide: 0,
        slides: [],
        init() {
            // Mengambil maksimal 3 gambar pertama untuk banner secara otomatis
    ...
    {{-- 🌌 HERO SECTION: DYNAMIC SLIDER (3 SLIDES) --}}
    <section class="relative h-[75vh] min-h-[500px] flex items-center bg-[#161f36] overflow-hidden">
        ...
        {{-- Overlays & Textures --}}
        <div class="absolute inset-0 bg-gradient-to-r from-[#161f36] via-[#161f36]/60 to-transparent z-10"></div>
        <div class="absolute inset-0 opacity-10 z-10" style="background-image: url('https://www.transparenttextures.com/patterns/carbon-fibre.png');"></div>
        ...
        <div class="container mx-auto px-6 relative z-20">
            <div class="max-w-4xl">
                ...
                <p class="text-slate-400 text-lg md:text-xl max-w-xl font-medium leading-relaxed border-l-4 border-[#FF7518] pl-6" data-aos="fade-up" data-aos-delay="200">
                    Dokumentasi infrastruktur dan aset PT. RBM. Bukti nyata komitmen kami dalam membangun konektivitas Indonesia.
                </p>
            </div>
        </div>
    </section>
