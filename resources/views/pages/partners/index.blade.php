    ...
    <div class="bg-white min-h-screen font-sans text-[#161f36] selection:bg-[#FF7518]/30"
        x-data="{
            ...
            isVisible(sector, name) {
                ...
                // Normalisasi string agar pencarian tidak sensitif huruf besar/kecil
                const cleanSector = sector.trim().toLowerCase();
                const cleanActive = this.activeSector.trim().toLowerCase();
                        ...
                        <div x-show="isVisible('{{ $partner->sector }}', '{{ $partner->name }}')"
                            ...
                            <div class="relative w-full h-48 mb-8 bg-slate-50 rounded-[2.5rem] p-8 flex items-center justify-center overflow-hidden border border-slate-50 transition-colors group-hover:bg-white">
                                ...
                                @if ($finalUrl)
                                    <img src="{{ $finalUrl }}"
                                        ...
                                        alt="{{ $partner->name }}"
                                        onerror="this.onerror=null; this.src='https://placehold.co/400x400?text=No+Logo';">
                                @else
                            ...
                            <div class="flex-grow flex flex-col">
                                <div class="flex items-center gap-3 mb-4">
                                    <span class="w-2 h-2 rounded-full {{ strtolower(trim($partner->sector)) === 'tower provider' ? 'bg-blue-500' : 'bg-[#FF7518]' }}"></span>
