...
@section('content')
    ...
    @php
        ...
        $getImg = function($path) {
            ...
            if (str_contains($path, 'assets/')) return asset($path);
            return asset('storage/' . $path);
        };

        $photos = $items->map(fn($item) => $getImg($item->image))->toArray();

        // Banner & Grid Fallbacks
    ...
    {{-- 🌌 1. HERO SLIDER --}}
    <section class="relative w-full h-[600px] overflow-hidden bg-[#161f36] font-['Plus_Jakarta_Sans']">
        <div x-data="{ activeSlide: 1, totalSlides: 2 }"
            ...
            <div x-show="activeSlide === 1"
                ...
                <img src="{{ $heroImage1 }}" class="w-full h-full object-cover opacity-40 scale-110 transition-transform duration-[15000ms] ease-in-out" :class="activeSlide === 1 ? 'scale-100' : 'scale-110'">
                <div class="absolute inset-0 bg-gradient-to-t from-[#161f36] via-transparent to-transparent md:bg-gradient-to-r flex items-center">
                    <div class="max-w-7xl mx-auto px-8 w-full">
                        <span class="inline-block px-4 py-1.5 bg-[#FF7518] text-white text-[10px] font-black uppercase tracking-[0.3em] mb-6 rounded-lg shadow-lg shadow-orange-600/20">Industrial Excellence</span>
                        <h2 class="text-white text-5xl md:text-8xl font-black uppercase leading-[0.9] max-w-3xl tracking-[-0.05em]">
                            Tower <br><span class="text-[#FF7518]">Construction</span>
                        </h2>
                    </div>
                </div>
            </div>
            ...
            <div x-show="activeSlide === 2"
                ...
                <img src="{{ $heroImage2 }}" class="w-full h-full object-cover opacity-40 scale-110 transition-transform duration-[15000ms] ease-in-out" :class="activeSlide === 2 ? 'scale-100' : 'scale-110'">
                <div class="absolute inset-0 bg-gradient-to-t from-[#161f36] via-transparent to-transparent md:bg-gradient-to-r flex items-center">
                    <div class="max-w-7xl mx-auto px-8 w-full">
                        <span class="inline-block px-4 py-1.5 bg-[#FF7518] text-white text-[10px] font-black uppercase tracking-[0.3em] mb-6 rounded-lg shadow-lg shadow-orange-600/20">Maximum Stability</span>
                        <h2 class="text-white text-5xl md:text-8xl font-black uppercase leading-[0.9] max-w-3xl tracking-[-0.05em]">
                            Expert <br><span class="text-[#FF7518]">Maintenance</span>
                        </h2>
                    </div>
                </div>
            </div>
